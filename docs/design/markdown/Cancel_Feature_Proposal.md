...
- 基本系列4: 予約が**要求者本人のものでない**場合は, キャンセルできない旨を表示して終了する (他人の予約は操作できない)。
...
| リポジトリ (契約・SQLite・スキーマ) | 変更 | `reservations` テーブルに `line_user_id` 列を追加し, 保存・復元する。契約の検索メソッドは変更なし。 |
| `ReservationControl.reserve_rooms()` | 変更 | 予約時に `line_user_id` を受け取り, 予約に保存する。 |
| `CancelControl` | 変更 | `search_reservation` / `cancel` に要求者の userId を渡し, **本人確認 (userId 一致)** を行う。不一致は `BureaucraticError`。 |
...
| テスト | 追加 | 本人確認 (他人の予約は不可), 期限 (当日以降は不可), 正常系のキャンセルを検証。 |
